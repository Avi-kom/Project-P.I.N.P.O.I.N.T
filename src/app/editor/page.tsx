    ...
    <div className="mx-auto flex w-full max-w-[1440px] flex-1 flex-col gap-6 p-4 sm:p-8">
      ...
      <div className="flex flex-nowrap items-start gap-6 overflow-x-auto">
        <div
          ...
          className="relative h-[400px] w-[1040px] max-w-full shrink-0 overflow-hidden rounded-lg border border-stone-400 bg-[#b8b2a1]"
        >
          {zones.map((zone) => (
            <div
              key={zone.id}
              ...
              className={cn(
                "absolute flex cursor-move items-center justify-center rounded-sm border p-1 text-center text-xs font-semibold shadow-sm select-none",
                zone.color,
                zone.labelColor,
                selectedId === zone.id
                  ? "border-2 border-dashed border-slate-900 ring-2 ring-slate-900/40"
                  : "border-black/30"
              )}
            >
              {zone.label}
              <div
                ...
                className="absolute right-0 bottom-0 h-3.5 w-3.5 cursor-nwse-resize rounded-tl bg-slate-900/70"
              />
            </div>
          ))}
        </div>

        <div className="w-72 shrink-0 space-y-4 rounded-lg border border-slate-700 bg-slate-900 p-4">
